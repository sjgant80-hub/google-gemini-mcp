#!/usr/bin/env node
// google-gemini-mcp · stdio MCP server exposing foldkit as Claude-callable tools + resources
...
const server = new Server(
  { name: 'google-gemini-mcp', version: '1.0.0' },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  try {
    ...
    return {
      isError: true,
      content: [{ type: 'text', text: `google-gemini-mcp error: ${err.message}` }]
    };
  }
});
